export async function onRequestPost(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    try {
        ...
        console.log('Upload medkits POST request received with token:', token);
        ...
        // Insert or update medkits used in database
        for (const [playerId, medkitsUsed] of Object.entries(requestData)) {
            await env["zeitvertreib-data"]
                .prepare(`INSERT INTO playerdata (id, usedmedkits) VALUES (?, ?) 
                         ON CONFLICT(id) DO UPDATE SET usedmedkits = COALESCE(usedmedkits, 0) + ?`)
                .bind(playerId, medkitsUsed, medkitsUsed)
                .run();
        }
        
        return Response.json({ success: true, message: "Medkits usage data updated successfully" });
        
    } catch (error) {
        console.error('Upload medkits POST error:', error);
