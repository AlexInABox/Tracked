export async function onRequestPost(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    try {
        ...
        console.log('Upload colas POST request received with token:', token);
        ...
        // Insert or update colas used in database
        for (const [playerId, colasUsed] of Object.entries(requestData)) {
            await env["zeitvertreib-data"]
                .prepare(`INSERT INTO playerdata (id, usedcolas) VALUES (?, ?) 
                         ON CONFLICT(id) DO UPDATE SET usedcolas = COALESCE(usedcolas, 0) + ?`)
                .bind(playerId, colasUsed, colasUsed)
                .run();
        }
        
        return Response.json({ success: true, message: "Colas usage data updated successfully" });
        
    } catch (error) {
        console.error('Upload colas POST error:', error);
