export async function onRequestPost(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    try {
        ...
        // Insert or update playerdata in database
        for (const [playerId, playtime] of Object.entries(requestData)) {
            await env["zeitvertreib-data"]
                .prepare(`INSERT INTO playerdata (id, playtime) VALUES (?, ?) 
                         ON CONFLICT(id) DO UPDATE SET playtime = playtime + ?`)
                .bind(playerId, playtime, playtime)
                .run();
        }
